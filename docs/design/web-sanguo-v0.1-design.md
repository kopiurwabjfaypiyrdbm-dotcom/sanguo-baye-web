...
> 方向校正（2026-07-24）：项目目标已从“保留策略味道的通用原型”收束为“以步步高电子词典版《三国霸业》为基线的现代 Web 重写”。本文中的临时公式、12 城示例和 AI 规则只用于验证架构，不代表原版已经还原。原版事实源、差异状态和美术来源边界以 `references/` 下文档为准。

## 目标

v0.1 的工程目标是用 Web 技术栈跑通一个可测试的战略循环，为后续逐模块还原原版规则建立边界清晰的现代架构。最终玩法以固定版本的 C 移植核心和可重复运行结果为基线，界面和交互采用现代 Web 方式呈现。
