...
| Android Debug APK | 安装/启动通过；物理触摸待验收 | MuMu 安装、启动、横屏、渲染、命令和跨进程存读已实跑；ADB/窗口注入不能替代 Android-first 真实点击、拖动、缩放和点选证据 |
...
- 曾记录过 `input tap` 点选路径，但 MB23/MB24 复核确认 MuMu 旋转窗口中的 ADB 触摸注入不能稳定命中 Godot 控件；该记录不作为 Android-first 物理触摸验收证据。
...
完成实现后分别进行了 Godot 架构与场景树、确定性规则与 fixture、Android/触控体验三路只读审查。场景树、确定性、恢复候选、安全区、终局回写和命令 ID 问题已回修并复核；当前没有 P0，代码静态 P1 已关闭，但 Android-first 真实点击、拖动、双指缩放、城池/单位点选与战术往返仍是 P1 人工验收缺口。presentation runner 与 full-loop runner 的剩余纵向冷恢复覆盖列入 MB25。
