...
- 导出：Godot 4.7.1 stable `a13da4feb`，GDScript，2026-08-04 07:21:30（使用现有本机 Android Debug 模板）
- 大小：57,813,476 bytes（约 55.1 MiB / 57.8 MB，不是 5 GB）
- SHA-256：`B7C49888A6F60DC7BC6B71B71EC5DCEFE64E8D967DC1FDFD3227CA4896AB3456`（`Get-FileHash`）
...
- 增量项目验证：`npm run godot:project:verify` 退出码 0（211 domain、216 presentation assertions）。当前源码的 Android preset 已使用项目自有 `res://icon.svg` 主图标及独立 adaptive foreground/background/monochrome SVG；触控度量纯函数覆盖 160/360 dpi 和 48dp/拖动阈值换算。
- 增量导出：沙箱上下文曾把已存在的模板误报为 missing；改用授权环境读取现有模板后导出成功。没有下载或安装组件。
- 触控增量后重新导出并安装了上述 APK；冷启动进程可见。完整 `npm run check` 在 553.4 秒后退出码 0，Godot/Web/构建门禁均通过；迁移回放中的错误仍是预期负向篡改演练。
