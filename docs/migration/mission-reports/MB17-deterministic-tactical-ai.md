...
本 Mission 的交叉语言验收采用时期 1、显式 seed 的真实 Web 轨迹：主案例验证曹操在 `(3,3)` 选择相邻守将耿武 `(3,4)`，造成 14 点兵力损失，经验增加 2，seed 不消耗，随后进入守方阶段；另外三个 policy case 分别覆盖 `rally` 自身、移动后攻击和移动后等待/占城终局。Godot 的命令 envelope、receipt、日志、完整战斗状态和 canonical SHA 与 TypeScript fixture 逐字段一致。
...
- `scripts/generate-godot-tactical-battle-ai-fixture.ts`、`godot/data/fixtures/tactical-battle-ai-v1.json`：由 `createTacticalBattle`、`runBasicTacticalAi`、攻击/移动/计谋/等待 API 生成的语言无关 fixture，包含主攻击轨迹、3 个 policy case、11 个事务和损坏边界。
...
- AI 阶段交接显式重置双方 `moved/acted`、重建 `actedUnitIds` 并追加“守方开始行动。”日志；没有把战斗状态挂到 Node/Control/场景树。

Godot runner 当前通过 53 项断言（11 个 boundary、主轨迹和 3 个 policy case）；既有战术 battle/movement/attack/skill runner 分别继续通过 70/54/56/54 项断言。
...
| `npm run godot:tactical-ai:verify` | 通过；11 个 boundary、4 条 Web policy 轨迹，53 项断言 |
...
| 三路只读审查 | 架构、确定性/fixture、Android/触控均 P0/P1/P2 为 0 |
| `npm run check` | 完成收尾复跑；Web、fixture、Godot、reference、测试和 build 必须保持绿色 |
...
本 Mission 只把当前 Web 的确定性战术 AI 语义接到四条可验证的有界轨迹，并覆盖 rally、移动候选和等待的编排入口；不宣称完整十项计谋、多回合粮草/天气、撤退、主将/全灭结算或战后战略回接已经迁移。AI 评分是当前 Web 现代规则，不是未经证实的 BBK 原版 AI ABI。战斗结算和战略状态回写由 MB19 负责。
