...
- **`gdeck_validate` 扩展**：`check` 支持 `file`（单文件秒级校验）；`verify` 支持 `json`（结构化失败摘要）；`oracle`（新增，跑 `npm run check` 的 Godot 侧 18 个 oracle 脚本 + reference:check，逐条计时 + 结构化报告，实测 19/19 通过约 461s。**⚠️ 调用时须传 `timeout: 900`**（扩展外层超时默认 300s 不够，上限 900s））。
