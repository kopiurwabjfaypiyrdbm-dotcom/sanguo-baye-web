...
- Quick exact-command editors remain node-anchored without resizing the map or displacing the global campaign dock. Parameter-heavy expanded editors may yield the dock and use the larger command workspace. Safe areas, short landscape viewports, and the on-screen keyboard do not conceal primary controls.
