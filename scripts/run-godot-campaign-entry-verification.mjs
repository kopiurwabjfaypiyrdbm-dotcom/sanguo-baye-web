...
if (version.error || version.status !== 0 || !/^4\.7\.1(?:\.|$)/u.test(versionText)) {
  throw new Error(`Godot 4.7.1 is required: ${versionText || version.error?.message || String(version.stderr ?? '').trim()}`);
}
process.stdout.write(`[Godot campaign entry] engine=${versionText}\n`);
const result = spawnSync(engine, [
  '--headless', '--display-driver', 'headless', '--path', resolve(root, 'godot'), '--script', 'res://tests/campaign_entry_runner.gd',
], { cwd: root, env, encoding: 'utf8', timeout: 180_000, windowsHide: true });
if (result.stdout) process.stdout.write(result.stdout);
if (result.stderr) process.stderr.write(result.stderr);
const output = `${result.stdout ?? ''}\n${result.stderr ?? ''}`;
if (/SCRIPT ERROR|Node not found|Parse Error/u.test(output)) throw new Error('Godot campaign-entry runner emitted a script/runtime error');
if (result.error || result.signal || result.status !== 0) {
  throw new Error(`Godot campaign-entry verification failed: ${result.error?.message ?? result.signal ?? result.status}`);
}
