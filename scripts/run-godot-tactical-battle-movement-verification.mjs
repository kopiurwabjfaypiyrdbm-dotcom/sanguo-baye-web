...
const runtime = resolve(project, '.godot/runtime-tactical-movement');
const paths = {
...
process.stdout.write(`[Godot tactical movement] engine=${versionText}\n`);
const result = spawnSync(
  engine,
  ['--headless', '--path', project, '--script', 'res://tests/tactical_battle_movement_runner.gd'],
  { cwd: root, env, encoding: 'utf8', timeout: 180_000 },
);
if (result.stdout) process.stdout.write(result.stdout);
if (result.stderr) process.stderr.write(result.stderr);
if (result.error || result.signal || result.status !== 0) {
  throw new Error(`Godot tactical movement verification failed: ${result.error?.message ?? result.signal ?? result.status}`);
}
