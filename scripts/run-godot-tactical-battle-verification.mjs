...
const runtime = resolve(project, '.godot/runtime-tactical');
const paths = {
...
process.stdout.write(`[Godot tactical battle] engine=${versionText}\n`);
const result = spawnSync(
  engine,
  ['--headless', '--path', project, '--script', 'res://tests/tactical_battle_runner.gd'],
  { cwd: root, env, encoding: 'utf8', timeout: 180_000 },
);
if (result.stdout) process.stdout.write(result.stdout);
if (result.stderr) process.stderr.write(result.stderr);
if (result.error || result.signal || result.status !== 0) {
  throw new Error(`Godot tactical battle verification failed: ${result.error?.message ?? result.signal ?? result.status}`);
}
