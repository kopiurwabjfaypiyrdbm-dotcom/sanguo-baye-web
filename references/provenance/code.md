...
| `src/core/strategicOrders.ts` 道路调动 | `baye/order.h:OrderType`、`citycmdc.c:MoveMake`、`citycmdd.c:AddOrderHead`、`citycmd.c:PolicyExec/MoveDrv`、`tactic.c` 月度阶段 | 依据命令字段、离城与抵达状态重写独立 schema 4 在途层；未复制控制流。固定 C 数组队列同月执行并忽略 `TimeCount`，旧递减分支已注释，且 `SearchRoad` 函数体不在 allowlist；因此排序 BFS、每段 1 月、4 体力和目标易主安全返回是明确的现代修复/临时规则 | `src/core/strategicOrders.test.ts`、`src/core/personnelCommands.test.ts`、中途存档往返测试 |
