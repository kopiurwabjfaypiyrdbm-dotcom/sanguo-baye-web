...
| `src/core/cityEvents.ts`、`src/core/economy.ts` 城市事件 | `infdeal.c:CitiesUpDataDate/RandEvents/EventStateDeal`、`tactic.c:ConditionUpdate`、`baye/attribute.h` | 依据月度调用顺序、季度防灾衰减、状态枚举、驻军损失、城市损失及随机比较方向重写；未复制报告界面。在途供养、可见日志与旧存档缺省正常为现代整合规则 | `src/core/cityEvents.test.ts`、`src/core/economy.test.ts`、`src/core/turn.test.ts`、`references/city-event-notes.md` |
