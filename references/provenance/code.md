...
| `godot/src/domain/progression/calendar_events.gd`、`annual_progression.gd` 城市与年度阶段 | 同上；直接行为 oracle 为本项目 TypeScript `cityEvents.ts`、`annualProgression.ts` | 以 GDScript 独立重写固定整数顺序、显式 `cityOrder`/`officerOrder`、条件式 LCG 调用、严格年份登场与日志 receipt；不链接 C/TypeScript，不复制原界面或资源。四时期没有未来道具日程的事实来自当前可再分发数据契约 | `application-session-suite-v1.json` MB11 序列、`godot/tests/run_all.gd`、`references/city-event-notes.md` |
...
| `godot/src/domain/progression/officer_lifecycle.gd`、`campaign_outcome.gd`、`godot/src/application/commands/succession_adapter.gd` | 同上；直接行为 oracle 为本项目 TypeScript `officerLifecycle.ts`、`outcome.ts` 与应用事务适配 | 以 GDScript 独立重写显式候选排序、俘虏逃脱/自然死亡政策、人物引用清理、玩家待继承/AI 自动继承/势力瓦解及结束态订单终止；不复制上游对话、控制流或素材。安全默认、现代逃脱、可恢复继承点与胜负关闭语义仍明确为产品规则 | `application-session-suite-v1.json` MB11 序列、Godot validator/domain/presentation 测试、`references/officer-lifecycle-notes.md` |
