...
| `src/core/cityCommands.ts` 招商、治理与出巡 | `baye/order.h`、`citycmdb.c:AccractbusinessMake/FatherMake/InspectionMake`、`citycmd.c:FatherDrv/InspectionDrv` | 依据智力增量、防灾/民忠随机区间、人口/属性上限及治理恢复正常状态重写即时城市命令；未复制菜单或控制流。灾害状态已接入，运行时成本表仍缺失，因此金钱/体力成本为明确临时规则 | `src/core/cityCommands.test.ts`、`src/core/ai.test.ts`、`references/city-command-notes.md` |
...
| `src/compat/baye/diplomacy.ts`、`src/core/diplomaticOrders.ts` 外交谋略 | `citycmd.c:AlienateDrv/CanvassDrv/CounterespiongeDrv/InduceDrv`、`citycmdc.c:AlienateMake/CanvassMake/CounterespiongeMake/InduceMake`、`tactic.c:ComputerTacticDiplomatism`、`baye/order.h`、`baye/attribute.h` | 重写四类判定的规则与隐藏对话随机调用、无符号整数宽度、严格比较、性格阈值和主要归属结果；未复制对话、菜单或控制流。一月耗时、4 体力、50 金、侦察名单锁定、动态势力 ID 与失地/在途安全闭包为明确现代规则 | `src/compat/baye/diplomacy.test.ts`、`src/core/diplomaticOrders.test.ts`、`references/diplomacy-notes.md` |
