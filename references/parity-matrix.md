...
| 城池命令 | `citycmd*.c` | `cityCommands.ts`（开垦、征兵、分配）；`personnelCommands.ts`（搜寻、登用、赏金、道具赏赐/卸装、道路调动、任命）；`strategicOrders.ts`（schema 4 在途命令）；`reconnaissance.ts`（侦察快照）；`captiveCommands.ts`（招降、释放） | 临时实现 | 调动已改为可保存的跨月道路命令：排序 BFS、每段 1 月、4 体力、目标易主安全返回均为现代规则。固定 C 的 `MoveMake` 会写 `TimeCount` 并让人物离城，但数组版 `PolicyExec` 同月执行且忽略该字段，旧递减代码已注释；`SearchRoad` 函数体也未入 allowlist，故不声明原版一致。输送与其余经营命令仍待实现 |
