...
| 城池命令 | `citycmd*.c` | `cityCommands.ts`（开垦、招商、治理、出巡、征兵、分配）；`personnelCommands.ts`（搜寻、登用、赏金、道具赏赐/卸装、道路调动、任命）；`strategicOrders.ts`（schema 4 调动/输送在途命令）；`reconnaissance.ts`（侦察快照）；`captiveCommands.ts`（招降、释放） | 临时实现 | 招商保留原版智力随机增量形状与商业上限；治理保留防灾 1–4/100 上限；出巡保留民忠 1–4、人口 100 与双上限。固定运行时成本表缺失，当前成本为可替换产品规则。调动/输送已成为可保存跨月道路命令；排序 BFS、每段 1 月、4 体力及易主后的状态闭包均为现代规则。交易、宴请、掠夺仍待实现 |
