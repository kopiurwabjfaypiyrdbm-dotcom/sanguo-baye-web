...
func _run() -> void:
	...
	for index: int in range(period_option.item_count):
		...
		_assert_true(ruler_option.selected < 0 and start_button.disabled, "period changes must keep ruler selection explicit")
	period_option.select(3)
	period_option.emit_signal("item_selected", 3)
	ruler_option.select(0)
	ruler_option.emit_signal("item_selected", 0)
