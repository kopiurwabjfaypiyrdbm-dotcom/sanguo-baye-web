...
func _run() -> void:
	...
	var end_turn_button: Button = screen.get_node("%DockEndMonthButton")
	var dock_intel_button: Button = screen.get_node("%DockIntelButton")
	_assert_true(dock_intel_button.visible, "strategy screen must expose the Web-aligned campaign dock")
	_assert_true(screen.get_node("%DockCitiesButton").visible, "campaign dock must expose cities entry")
	_assert_true(screen.get_node("%DockOfficersButton").visible, "campaign dock must expose officers entry")
