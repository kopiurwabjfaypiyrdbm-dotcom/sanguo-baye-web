...
const Commands = preload("res://src/domain/commands/reconnaissance_commands.gd")
const StringContract = preload("res://src/application/commands/application_string_contract.gd")

const PARAMETER_KEYS: Array[String] = ["sourceCityId", "targetCityId", "officerId"]


static func validate_parameters(kind: String, parameters: Dictionary) -> Dictionary:
	if kind != "reconnoitre_city":
		return _failure("unsupported reconnaissance command: %s" % kind)
