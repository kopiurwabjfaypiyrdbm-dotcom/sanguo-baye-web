...
当前 React 页面已经接入参考原版流程的“新君登基 → 选择时期 → 选择君主”开局界面，以及可拖动、缩放和选择城池的 Phaser 战略地图。董卓弄权、曹操崛起、赤壁之战和三国鼎立四个时期都已转换为随应用发布的结构化数据；玩家无需选择本地文件即可进入 38 城完整地图。原始资源库和来源不明的图片、字体没有进入发布包。开垦、招商、治理、出巡、交易、宴请、掠夺、征兵、搜寻、登用、赏金、道具赏赐与双槽装备、侦察、离间/招揽/策反/劝降、战后招降/释放/处斩/流放、人物流放与装备没收、道路调动、资源输送、太守任命、出征和月度推进已经接入。调动、输送与谋略会跨月执行，执行者期间不会重复驻城或行动，保存重载与目标变化均有状态闭包。谋略必须先取得本回合侦察人物名单，界面不显示精确成功率；四项判定保留固定 C 的随机比较和性格阈值，一月耗时、4 体力与 50 金属于可替换现代规则。交易提供精确资源预览，宴请不占用或重置行动，掠夺必须经过危险后果确认。饥荒、旱灾、水灾和暴动会跨月持续并影响资源、人口与驻军；治理、出巡、交易和 AI 后勤可以实际应对。跨入 1 月时全员年龄增长，满足稀疏原版日程的隐藏人物进入可搜索在野状态，旧 schema 4 战役会在载入时幂等补齐日程。开局可锁定战死、自然死亡和俘虏逃脱政策；安全默认只增长年龄，其余关闭。人物死亡会回收装备、修复太守和取消关联命令；玩家君主失效时进入可保存的继承选择，AI 自动继承，无人可继则势力瓦解。敌城不会再免费展示动态资源和驻军；玩家可由武将消耗行动取得并长期保留当月情报快照。四时期的城池隐藏道具队列和人物初始装备会随剧本载入；现有 33 项道具内容沿用仓库早期整理表，固定上游重新关联前按临时产品数据处理。同类型道具可以存在多个唯一实例，但同一实例不能同时位于人物与城池。搜寻可发现道具，装备加成会进入战略与手动战斗计算。快速与手动战斗共用确定性俘虏、可选战死与经验升级回写，AI 也会经营本城俘虏、按真实供养口径买粮并实施道路后勤。出征可选择快速结算或进入 Phaser 手动战场，完成单位移动、普通攻击、火计/扰乱/激励、天气与混乱、待命、敌方计谋 AI、粮草与胜负结算后返回战略地图；AI 进攻玩家城市时也会暂停月度推演并允许玩家手动防守。
